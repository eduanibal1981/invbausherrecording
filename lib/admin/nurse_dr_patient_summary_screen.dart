...
class NurseDrPatientSummaryScreen extends StatefulWidget {

  const NurseDrPatientSummaryScreen({super.key});



  @override

  State<NurseDrPatientSummaryScreen> createState() =>

      _NurseDrPatientSummaryScreenState();

}



class _NurseDrPatientSummaryScreenState

    extends State<NurseDrPatientSummaryScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchSummary() async {
    ...
    try {
      ...
      final response = await Supabase.instance.client.rpc(
        ...
        'get_nurseanddr_patient_summary',
      ...
      final filteredData = data.where((row) {
        ...
        if (row['staff_name'] == 'TOTAL') return true;
      ...
      filteredData.sort((a, b) {
        ...
        if (a['staff_name'] == 'TOTAL') return -1;

        if (b['staff_name'] == 'TOTAL') return 1;
      ...
      if (e.toString().contains('get_nurseanddr_patient_summary')) {

        throw Exception(

          'Database function get_nurseanddr_patient_summary is missing. '
  ...
  bool _isOnLeaveNurseRow(
    ...
    final staffName = (row['staff_name'] ?? '').toString().trim().toLowerCase();

    if (staffName.isNotEmpty && onLeaveNames.contains(staffName)) {
    ...
    for (final key in ['medicalstaffid', 'staffid', 'staff_id', 'nstaffid']) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Nurse & Doctor Patient Summary'),
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          var nurseRows = allData
              ...
              .where((row) => row['staff_name'] != 'TOTAL')
          ...
          nurseRows.sort((a, b) {
            ...
            final nameA = (a['staff_name'] ?? '').toString().toLowerCase();

            final nameB = (b['staff_name'] ?? '').toString().toLowerCase();
